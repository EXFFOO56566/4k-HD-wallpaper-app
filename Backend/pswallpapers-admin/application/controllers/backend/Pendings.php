...
class Pendings extends BE_Controller {
	...
	/**
	 * List down the pending wallpapers
	 */
	function index() {
		// no publish filter
		...
		$pendings = $this->Pending->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
	...
	function edit( $id ) {
		...
		// load user
		$pending = $this->Pending->get_one( $id );

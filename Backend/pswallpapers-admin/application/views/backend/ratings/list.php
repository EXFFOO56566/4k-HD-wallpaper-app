<div class="table-responsive animated fadeInRight">
	<table class="table m-0 table-striped">
		<tr>
			...
			<th><?php echo get_msg('rating_count'); ?></th>
		...
		<?php if ( !empty( $ratings ) && count( $ratings->result()) > 0 ): ?>
			...
			<?php foreach($ratings->result() as $rate): ?>
				...
				<tr>
					...
					<td><?php echo $this->Wallpaper->get_one($rate->wallpaper_id)->wallpaper_name;?></td>
					
					<td><?php 
						echo $rate->overall_rating;

					?></td>
					<?php 

						$default_photo = get_default_photo( $rate->wallpaper_id, 'wallpaper' );
						$default_video_icon = get_default_photo( $rate->wallpaper_id, 'video-icon' );
